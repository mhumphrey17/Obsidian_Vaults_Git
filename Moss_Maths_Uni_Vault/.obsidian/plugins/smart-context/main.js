...
default: () => SmartContextEarlyPlugin
});
...
const matching_file_paths = this.file_paths.filter((path3) => path3.includes(link_target));
...
function deep_merge_no_overwrite(target, source, path3 = []) {
if (!is_plain_object(target) || !is_plain_object(source)) {
return target;
}
if (path3.includes(source)) {
return target;
}
path3.push(source);
...
deep_merge_no_overwrite(target[key], val, [...path3]);
...
get env_data_dir() {
const env_settings_files = this.fs.file_paths?.filter((path3) => path3.endsWith("smart_env.json")) || [];
let env_data_dir = ".smart-env";
if (env_settings_files.length > 0) {
if (env_settings_files.length > 1) {
const env_data_dir_counts = env_settings_files.map((path3) => {
const dir = path3.split("/").slice(-2, -1)[0];
return {
dir,
...
* @returns {*}
*/
get_by_path(obj, path3, settings_scope = null) {
return get_by_path(obj, path3, settings_scope);
}
/**
...
*/
set_by_path(obj, path3, value, settings_scope = null) {
set_by_path(obj, path3, value, settings_scope);
}
/**
...
*/
delete_by_path(obj, path3, settings_scope = null) {
delete_by_path(obj, path3, settings_scope);
}
/**
...
const path3 = elm.dataset.smartSetting;
if (!path3) return;
...
this.set_by_path(scope.settings, path3, newValue);
});
}
});
}
...
function get_by_path(obj, path3, settings_scope = null) {
if (!path3) return "";
...
function set_by_path(obj, path3, value, settings_scope = null) {
const keys = path3.split(".");
if (settings_scope) {
keys.unshift(settings_scope);
}
...
function delete_by_path(obj, path3, settings_scope = null) {
const keys = path3.split(".");
if (settings_scope) {
keys.unshift(settings_scope);
}
...
*/
handle_folder_select(path3, value, elm, scope) {
...
* @param {object} scope - The current scope containing settings and actions.
*/
handle_file_select(path3, value, elm, scope) {
...
*/
revert_setting(path3, elm, scope) {
...
elm.innerHTML = "";
const path3 = elm.dataset.setting;
const scope = opts.scope || this.main.main;
const settings_scope = opts.settings_scope || null;
try {
let value = elm.dataset.value ?? this.main.get_by_path(scope.settings, path3, settings_scope);
...
this.main.set_by_path(scope.settings, path3, value, settings_scope);
}
...
const setting = renderer.call(this, elm, path3, value, scope, settings_scope);
...
this.add_button_if_needed(setting, elm, path3, scope);
this.handle_disabled_and_hidden(elm);
return elm;
} catch (e) {
console.error({ path: path3, elm });
console.error(e);
}
}
render_dropdown_component(elm, path3, value, scope, settings_scope) {
...
this.handle_on_change(path3, value2, elm, scope, settings_scope);
});
dropdown.setValue(value);
});
return smart_setting;
}
render_text_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
button.onClick(async () => this.handle_on_change(path3, text.getValue(), elm, scope));
});
} else {
text.onChange(async (value2) => {
clearTimeout(debounceTimer);
debounceTimer = setTimeout(() => this.handle_on_change(path3, value2.trim(), elm, scope, settings_scope), 2e3);
});
}
});
return smart_setting;
}
render_password_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
smart_setting.addText((text) => {
...
text.onChange(async (value2) => {
clearTimeout(debounceTimer);
debounceTimer = setTimeout(() => this.handle_on_change(path3, value2, elm, scope, settings_scope), 2e3);
});
});
return smart_setting;
}
render_number_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
clearTimeout(debounceTimer);
debounceTimer = setTimeout(() => this.handle_on_change(path3, parseInt(value2), elm, scope, settings_scope), 2e3);
});
});
return smart_setting;
}
render_toggle_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
toggle.onChange(async (value2) => this.handle_on_change(path3, value2, elm, scope, settings_scope));
});
return smart_setting;
}
render_textarea_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
render_textarea_array_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
smart_setting.addTextArea((textarea) => {
textarea.setPlaceholder(elm.dataset.placeholder || "");
...
render_button_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
smart_setting.addButton((button) => {
...
if (callback) callback(path3, value, elm, scope, settings_scope);
}
});
});
return smart_setting;
}
render_remove_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
smart_setting.addButton((button) => {
button.setButtonText(elm.dataset.btnText || elm.dataset.name || "Remove");
button.onClick(async () => {
this.main.delete_by_path(scope.settings, path3, settings_scope);
if (elm.dataset.callback) {
const callback = this.main.get_by_path(scope, elm.dataset.callback);
if (callback) callback(path3, value, elm, scope, settings_scope);
}
...
render_folder_select_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
this.handle_folder_select(path3, value, elm, scope);
});
folder_select.inputEl.querySelector("input").addEventListener("change", (e) => {
const folder = e.target.value;
this.handle_on_change(path3, folder, elm, scope, settings_scope);
console.log("folder changed", folder);
});
});
return smart_setting;
}
render_file_select_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
this.handle_file_select(path3, value, elm, scope, settings_scope);
});
});
return smart_setting;
}
render_slider_component(elm, path3, value, scope, settings_scope) {
const smart_setting = new this.setting_class(elm);
...
this.handle_on_change(path3, numericVal, elm, scope, settings_scope);
});
});
return smart_setting;
}
render_html_component(elm, path3, value, scope) {
elm.innerHTML = value;
return elm;
}
add_button_if_needed(smart_setting, elm, path3, scope) {
if (elm.dataset.btn) {
smart_setting.addButton((button) => {
...
else scope[elm.dataset.btnCallback](path3, null, smart_setting, scope);
...
this.main.get_by_path(scope, elm.dataset.callback)(path3, null, smart_setting, scope);
...
handle_on_change(path3, value, elm, scope, settings_scope) {
this.pre_change(path3, value, elm, scope);
if (elm.dataset.validate) {
const valid = this[elm.dataset.validate](path3, value, elm, scope);
if (!valid) {
elm.querySelector(".setting-item").style.border = "2px solid red";
this.revert_setting(path3, elm, scope);
return;
}
}
this.main.set_by_path(scope.settings, path3, value, settings_scope);
if (elm.dataset.callback) {
const callback = this.main.get_by_path(scope, elm.dataset.callback);
if (callback) callback(path3, value, elm, scope);
}
this.post_change(path3, value, elm, scope);
}
render_button_with_confirm_component(elm, path3, value, scope) {
const smart_setting = new this.setting_class(elm);
smart_setting.addButton((button) => {
button.setButtonText(elm.dataset.btnText || elm.dataset.name);
...
async render_file_select_component(elm, path3, value) {
return super.render_text_component(elm, path3, value);
}
async render_folder_select_component(elm, path3, value) {
return super.render_text_component(elm, path3, value);
}
...
new import_obsidian7.Notice(update_notice, 0);
...
async load() {
if (import_obsidian7.Platform.isMobile && !this.manual_load) {
...
if (file instanceof import_obsidian7.TFile && this.smart_sources?.source_adapters?.[file.extension]) {
...
var LinkDepthModal = class extends import_obsidian8.SuggestModal {
/**
...
new import_obsidian8.Notice("Copied context to clipboard!");
}
};
...
var glob_to_regex_pattern3 = (pattern, extended_glob) => {
let in_class = false;
let in_brace = 0;
let result = "";
for (let i = 0; i < pattern.length; i++) {
...
var adjust_for_windows_paths3 = (pattern, windows_paths) => windows_paths ? pattern.replace(/\\\//g, "[\\\\/]") : pattern;
var create_regex3 = (pattern, { case_sensitive, extended_glob, windows_paths }) => {
...
function match_glob2(pattern, str, options = {}) {
...
if (match_glob2(pattern, last_heading, { case_sensitive: false })) {
...
item.data.outlinks = get_markdown_links3(content);
}
...
const path3 = this.get_item_data_path();
...
const raw_data = await this.fs.read(path3, "utf-8", { no_cache: true });
if (!raw_data) {
for (const item of Object.values(this.collection.items)) {
if (item._queue_load) {
item.queue_import?.();
...
if (rewrite) {
if (file_data.length) {
await this.fs.write(path3, file_data);
} else {
...
for (const [path3, item] of Object.entries(items)) {
const placeholders = build_item_placeholders(path3, depth);
chunks.push({
path: path3,
mtime: item.mtime,
...
function build_item_placeholders(path3, depth, mtime) {
const name = path3.substring(path3.lastIndexOf("/") + 1);
...
var SmartContextSettingTab = class extends import_obsidian9.PluginSettingTab {
...
var SmartContextPlugin = class extends import_obsidian10.Plugin {
...
onload() {
SmartEnv2.create(this, this.smart_env_config);
...
new import_obsidian10.Notice("Unable to copy text: no valid method found.");
}
} catch (err) {
console.error("Failed to copy text:", err);
new import_obsidian10.Notice("Failed to copy.");
}
}
/**
...
new import_obsidian10.Notice(noticeMsg);
}
};
...
var glob_to_regex_pattern4 = (pattern, extended_glob) => {
let in_class = false;
let in_brace = 0;
let result = "";
for (let i = 0; i < pattern.length; i++) {
...
var adjust_for_windows_paths4 = (pattern, windows_paths) => windows_paths ? pattern.replace(/\\\//g, "[\\\\/]") : pattern;
var create_regex4 = (pattern, { case_sensitive, extended_glob, windows_paths }) => {
const regex_pattern = glob_to_regex_pattern4(pattern, extended_glob);
const adjusted_pattern = adjust_for_windows_paths4(regex_pattern, windows_paths);
const flags = case_sensitive ? "" : "i";
return new RegExp(`^${adjusted_pattern}$`, flags);
};
function glob_to_regex4(pattern, options = {}) {
const default_options = { case_sensitive: true, extended_glob: false, windows_paths: false };
const merged_options = { ...default_options, ...options };
if (pattern === "") {
return /^$/;
...
return create_regex4(pattern, merged_options);
}
function match_glob3(pattern, str, options = {}) {
const regex = glob_to_regex4(pattern, options);
return regex.test(str);
}
...
function should_ignore(relative_path, patterns, aggregator = []) {
...
if (match_glob3(pat, relative_path, { case_sensitive: true })) {
aggregator.push(pat);
return true;
}
}
...
try {
await import_fs.promises.access(file_path);
return true;
} catch (err) {
return false;
}
...
const entries = await import_fs.promises.readdir(current_path, { withFileTypes: true });
for (const entry of entries) {
const full_path = import_path.default.join(current_path, entry.name);
const relative_path = import_path.default.relative(dir_path, full_path).replace(/\\/g, "/");
...
const ignore_path = import_path.default.join(base_path, ignore_file);
if (await exists_path(ignore_path)) {
const content = await import_fs.promises.readFile(ignore_path, "utf-8");
...
async function parse_codeblock(fileContent, basePath, options = {}) {
...
const abs_path = import_path.default.join(basePath, line);
...
const stats = await import_fs.promises.stat(abs_path);
...
const full_path = import_path.default.join(abs_path, f_obj.path).replace(/\\/g, "/");
if (includeNonText || is_text_file2(full_path, import_path.default.sep)) {
all_paths.push(full_path);
}
}
} else {
console.log("isFile", abs_path);
const relative_path = import_path.default.relative(basePath, abs_path).replace(/\\/g, "/");
if (should_ignore(relative_path, ignore_patterns, ignored_patterns_matched)) {
console.log("file ignored by ignore patterns", abs_path);
continue;
}
if (includeNonText || is_text_file2(abs_path, import_path.default.sep)) {
...
const content = await import_fs.promises.readFile(ext_path, "utf-8");
items[ext_path] = {
path: ext_path,
content,
...
function approximate_tokens2(charCount) {
return Math.ceil(charCount / 4);
}
var LinkDepthModal2 = class extends import_obsidian12.SuggestModal {
/**
...
this.zero_tokens = approximate_tokens2(this.zero_char_count);
const cache = sc_item?.meta?.depth_cache || null;
const isCacheValid = cache && cache.depth0_token_count === this.zero_tokens;
if (isCacheValid) {
this.depths_info = cache.depths_info;
...
const total_tokens = approximate_tokens2(total_chars);
const chars = total_chars > 1e4 ? `${Math.round(total_chars / 1e3)}k` : total_chars;
const tokens = total_tokens > 1e4 ? `${Math.round(total_tokens / 1e3)}k` : total_tokens;
let label = `Depth ${d} (${chars} chars, ${tokens} tokens)`;
if (total_tokens > 5e4) {
...
*/
async onChooseSuggestion(item) {
console.log("onChooseSuggestion", item, this.external_items);
const copying_notice = new import_obsidian12.Notice("Copying context...", 0);
...
function get_relative_path_to_vault(vault_path, targetPath) {
const relativePath = import_path2.default.relative(vault_path, targetPath);
...
let children;
try {
children = import_fs2.default.readdirSync(directoryPath, { withFileTypes: true });
} catch (_) {
return items;
}
for (const dirent of children) {
const fullPath = import_path2.default.join(directoryPath, dirent.name);
const relativePath = import_path2.default.relative(rootPath, fullPath).replace(/\\/g, "/");
const isDirectory = dirent.isDirectory();
if (!isDirectory && !is_text_file2(fullPath)) {
continue;
}
if (should_ignore(relativePath, [".git", "node_modules", "package-lock.json", ".obsidian", ".smart-env"])) {
continue;
}
items.push({
fullPath,
...
var ExternalSelectModal = class extends import_obsidian13.FuzzySuggestModal {
/**
* @param {import("obsidian").App} app
* @param {string} initialScope - The absolute path to start listing items
* @param {string} vault_path - The absolute path to the vault folder
*/
constructor(plugin) {
super(plugin.app);
this.plugin = plugin;
this.app = plugin.app;
this.vault_path = (0, import_obsidian13.normalizePath)(this.app.vault.adapter.basePath);
this.base_path = import_path2.default.dirname(this.vault_path);
console.log({ base_path: this.base_path, vault_path: this.vault_path });
this.insert_dir = false;
this.prevent_close = false;
this.setInstructions([
{ command: "Enter", purpose: "Insert external context" },
{ command: "\u2318/Ctrl + Enter", purpose: "Insert folder as external context" },
{ command: "\u2192", purpose: "Open directory" },
{ command: "\u2190", purpose: "Parent directory" },
...
} else if (e.key === "Enter" && import_obsidian13.Keymap.isModEvent(e)) {
e.preventDefault();
this.prevent_close = true;
this.insert_dir = true;
this.selectActiveSuggestion(e);
...
/**
* Gather items up to depth=2 under this.base_path.
* Also add '..' if not at system root.
* @returns {FileItem[]}
*/
getItems() {
const items = [];
const { root } = import_path2.default.parse(this.base_path);
if (this.base_path !== root) {
items.push({
fullPath: import_path2.default.join(this.base_path, ".."),
relativePath: "..",
isDirectory: true
});
}
const listing = list_directory_contents_up_to_depth(this.base_path, this.base_path, 0, 2);
items.push(...listing);
return items;
}
/**
...
if (isUpOne) {
this.base_path = import_path2.default.join(this.base_path, "..");
this.open();
return;
}
if (item.isDirectory && !this.insert_dir) {
this.base_path = item.fullPath;
this.open();
return;
}
const activeFile = this.app.workspace.getActiveFile();
if (!activeFile) {
new import_obsidian13.Notice("No active file is open.");
return;
}
const relPath = get_relative_path_to_vault(this.vault_path, item.fullPath);
this.insert_into_smart_context(activeFile, relPath).then(() => {
this.insert_dir = false;
new import_obsidian13.Notice(`Inserted ${item.isDirectory ? "folder" : "file"} path: ${relPath}`);
}).catch((err) => {
new import_obsidian13.Notice(`Error inserting path: ${err}`);
});
this.open();
}
/**
...
var BuildContextModal = class extends import_obsidian14.FuzzySuggestModal {
/**
...
(async () => {
const external_items = await parse_codeblock(await source_item.read(), this.app.vault.adapter.basePath);
for (const item of Object.values(external_items.items)) {
if (!this.selected_items.some((x) => x.path === item.path)) {
...
*/
getItems() {
const context_items = Object.values(this.env.smart_sources.items).filter((i) => this.selected_items.length === 0 || !this.selected_items.some((x) => x.path === i.path));
;
...
const { stats } = await this.compile_context();
const total_char_count = stats.char_count;
if (this.selected_items.length > 0) {
const char_count_display = this.selected_container_el.createDiv("st-build-context-char-count");
const formatted_count = total_char_count > 1e4 ? `${Math.round(total_char_count / 1e3)}k` : total_char_count;
char_count_display.createSpan({
text: `Estimated total: ${formatted_count} characters (${Math.ceil(total_char_count / 4)} tokens)`
});
char_count_display.style.fontWeight = "bold";
}
...
const path3 = e.target.dataset.path;
console.log("should remove", path3);
this.selected_items = this.selected_items.filter((x) => x.path !== path3);
this.render_pills();
});
(0, import_obsidian14.setIcon)(pill.createSpan({ cls: "st-build-context-pill-icon" }), "document");
}
}
/**
...
*/
async submit() {
const { context, stats } = await this.compile_context();
await this.plugin.copy_to_clipboard(context);
this.plugin.showStatsNotice(
stats,
...
async compile_context() {
const source_keys = this.selected_items.filter((x) => x.key).map((x) => x.key);
if (!source_keys.length) {
new import_obsidian14.Notice("No files selected.");
return;
}
const external_items = this.selected_items.filter((x) => !x.key);
const context_item = await this.env.smart_contexts.create_or_update({
context_items: source_keys.reduce((acc, key) => {
acc[key] = true;
return acc;
}, {})
});
return await context_item.compile({
items: external_items.reduce((acc, item) => {
acc[item.path] = item;
return acc;
}, {}),
link_depth: 0
...
var FolderSelectModal = class extends import_obsidian15.SuggestModal {
...
var SmartContextEarlyPlugin = class extends SmartContextPlugin {
LinkDepthModal = LinkDepthModal2;
register_commands() {
...
if (!checking) {
new import_obsidian16.Notice("This command is only available on desktop.");
}
return false;
}
if (!checking) {
...
callback: () => {
const modal = new BuildContextModal(this.app, this);
modal.open();
}
});
this.addCommand({
id: "copy-current-note-as-image",
name: "Copy Current Note as Image",
checkCallback: (checking) => {
const mdView = this.app.workspace.getActiveViewOfType(import_obsidian16.MarkdownView);
if (!mdView) return false;
if (this.app.isMobile) return false;
if (checking) return true;
this.copy_current_note_as_image();
return true;
}
});
}
open_external_file_modal() {
const modal = new ExternalSelectModal(this);
modal.open();
}
/**
* @method copy_current_note_as_image
* @description Captures the note content directly from the editor view.
*  Uses precise coordinate calculation to exclude gutters and capture only content.
*  Only runs on desktop; does nothing on mobile.
*/
async copy_current_note_as_image() {
if (this.app.isMobile) {
new import_obsidian16.Notice("Copy as image is only available on desktop.");
return;
}
const mdView = this.app.workspace.getActiveViewOfType(import_obsidian16.MarkdownView);
if (!mdView) {
new import_obsidian16.Notice("No active Markdown note to capture.");
return;
}
let clipboard, nativeImage, getCurrentWindow;
try {
({ clipboard, nativeImage } = window.require("electron"));
({ getCurrentWindow } = window.require("@electron/remote"));
} catch (error) {
console.error("Electron API access error:", error);
new import_obsidian16.Notice("Electron interface not available. Cannot capture screenshot.");
return;
}
const current_window = getCurrentWindow();
if (!current_window) {
new import_obsidian16.Notice("Cannot detect current window for capturing.");
return;
}
const { webContents } = current_window;
if (!webContents) {
new import_obsidian16.Notice("No webContents available for screenshot.");
return;
}
try {
new import_obsidian16.Notice("Preparing to capture note as image...");
...
`;
style.setAttribute("id", "smart-context-early-obsidian-style");
document.head.appendChild(style);
const editor_info = await webContents.executeJavaScript(`
(function() {
try {
const contentContainer = document.querySelector('.workspace-leaf.mod-active .markdown-source-view');
if (!contentContainer) return {error: "Cannot find editor container"};

const contentEl = contentContainer.querySelector('.cm-content');
if (!contentEl) return {error: "Cannot find content element"};

const scroller = contentContainer.querySelector('.cm-scroller');
if (!scroller) return {error: "Cannot find scroller element"};

const gutters = contentContainer.querySelector('.cm-gutters');
...
const gutterRect = gutters ? gutters.getBoundingClientRect() : {width: 0, right: containerRect.left};

const contentAreaLeft = Math.max(gutterRect.right, contentRect.left);
const contentWidth = containerRect.right - contentAreaLeft;

const lines = contentEl.querySelectorAll('.cm-line');
const lineCount = lines.length;

return {
contentLeft: contentAreaLeft,
...
console.error("Error measuring content:", err);
return {error: err.toString()};
}
})();
`);
if (editor_info.error) {
throw new Error(`Failed to analyze editor layout: ${editor_info.error}`);
}
const original_scroll2 = editor_info.currentScrollTop;
const content_left = Math.round(editor_info.contentLeft);
const content_top = Math.round(editor_info.contentTop);
const content_width = Math.round(editor_info.contentWidth);
const viewport_height = Math.round(editor_info.visibleHeight);
const overlap_ratio = 0.5;
const step_size = Math.floor(viewport_height * (1 - overlap_ratio));
const estimated_height = editor_info.scrollHeight + 500;
const total_steps = Math.ceil(estimated_height / step_size) + 2;
const captures = [];
for (let i = 0; i < total_steps; i++) {
const scroll_pos = i * step_size;
await webContents.executeJavaScript(`
document.querySelector('.cm-scroller').scrollTop = ${scroll_pos};
`);
...
if (!image.isEmpty()) {
captures.push(image);
new import_obsidian16.Notice(`Captured segment ${i + 1} of ${total_steps}`);
}
}
await webContents.executeJavaScript(`
(function() {
const scroller = document.querySelector('.cm-scroller');
scroller.scrollTop = 999999; // Very large value to ensure bottom
})();
`);
await new Promise((r) => setTimeout(r, 1e3));
const final_capture = await webContents.capturePage({
x: content_left,
y: content_top,
width: content_width,
height: viewport_height + 100
});
if (!final_capture.isEmpty()) {
captures.push(final_capture);
new import_obsidian16.Notice("Captured absolute bottom of document");
}
await webContents.executeJavaScript(`
document.querySelector('.cm-scroller').scrollTop = ${original_scroll2};
`);
if (captures.length === 0) {
throw new Error("Failed to capture any content");
}
const image_width = captures[0].getSize().width;
const unique_content_height = Math.floor(viewport_height * (1 - overlap_ratio));
let total_height = captures[0].getSize().height;
for (let i = 1; i < captures.length - 1; i++) {
total_height += unique_content_height;
}
total_height += captures[captures.length - 1].getSize().height;
total_height += 100;
const final_buffer = Buffer.alloc(image_width * total_height * 4);
let first_img = captures[0];
let first_size = first_img.getSize();
let first_buffer = first_img.toBitmap();
let row_size = first_size.width * 4;
for (let row = 0; row < first_size.height; row++) {
const src_start = row * row_size;
const dest_start = row * row_size;
if (src_start + row_size <= first_buffer.length && dest_start + row_size <= final_buffer.length) {
first_buffer.copy(final_buffer, dest_start, src_start, src_start + row_size);
}
}
let current_position = first_size.height;
for (let i = 1; i < captures.length - 1; i++) {
const img = captures[i];
const size = img.getSize();
const buffer = img.toBitmap();
const skip_rows = Math.floor(size.height * 0.6);
for (let row = skip_rows; row < size.height; row++) {
const src_start = row * row_size;
const dest_start = (current_position + (row - skip_rows)) * row_size;
if (src_start + row_size <= buffer.length && dest_start + row_size <= final_buffer.length) {
buffer.copy(final_buffer, dest_start, src_start, src_start + row_size);
}
}
current_position += size.height - skip_rows;
}
const last_img = captures[captures.length - 1];
const last_size = last_img.getSize();
const last_buffer = last_img.toBitmap();
const last_skip_rows = Math.min(Math.floor(last_size.height * 0.4), last_size.height / 2);
for (let row = last_skip_rows; row < last_size.height; row++) {
const src_start = row * row_size;
const dest_start = (current_position + (row - last_skip_rows)) * row_size;
if (src_start + row_size <= last_buffer.length && dest_start + row_size <= final_buffer.length) {
last_buffer.copy(final_buffer, dest_start, src_start, src_start + row_size);
}
}
const actual_height = current_position + (last_size.height - last_skip_rows);
const final_image = nativeImage.createFromBuffer(final_buffer, {
width: image_width,
height: Math.min(actual_height, total_height)
});
clipboard.writeImage(final_image);
new import_obsidian16.Notice("\u2713 Complete note image copied to clipboard!");
} catch (err) {
console.error("Failed capturing note image:", err);
new import_obsidian16.Notice(`Screenshot failed: ${err.message || "Unknown error"}`);
} finally {
document.querySelector(".status-bar").style.display = "block";
const style = document.getElementById("smart-context-early-obsidian-style");
if (style) style.remove();
try {
webContents.executeJavaScript(`
document.querySelector('.cm-scroller').scrollTop = ${original_scroll || 0};
`);
} catch (e) {
}
}
