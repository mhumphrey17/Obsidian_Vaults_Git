...
ScEarlySettingsTab: () => ScEarlySettingsTab,
default: () => SmartConnectionsEarlyReleasePlugin
});
...
if (!entity)
return false;
if (entity.is_canvas || entity.is_excalidraw)
return true;
return false;
}
...
if (item.source_adapter.should_import)
item.queue_import();
else if (item.should_embed)
item.queue_embed();
}
}
/**
...
if (!["png", "jpg", "jpeg"].some((ext) => this.env.opts.collections?.[this.collection_key]?.source_adapters?.[ext]))
delete _settings_config.enable_image_adapter;
if (!this.env.opts.collections?.[this.collection_key]?.source_adapters?.["pdf"])
delete _settings_config.enable_pdf_adapter;
...
if (item.data.last_import?.at)
item.data.last_import.at = 0;
item.queue_import();
item.queue_embed();
...
var set_connections_pro_dom_effect = import_state.StateEffect.define();
var local_storage_key = "sc_connections_folded";
var connections_dom_plugin = import_view.ViewPlugin.fromClass(
class {
/**
...
constructor(view) {
this.view = view;
this.connections_pro_frag = null;
this.init();
}
init() {
this.folded = localStorage.getItem(local_storage_key) === "true";
this.container_el = document.createElement("div");
this.container_el.classList.add("embedded-backlinks");
this.nav_header_el = document.createElement("div");
this.nav_header_el.classList.add("nav-header");
this.container_el.appendChild(this.nav_header_el);
this.nav_buttons_container = document.createElement("div");
this.nav_buttons_container.classList.add("nav-buttons-container");
this.nav_header_el.appendChild(this.nav_buttons_container);
this.collapsed_notice_el = document.createElement("button");
this.collapsed_notice_el.classList.add("collapsed-notice");
this.collapsed_notice_el.textContent = "Collapsed (click to expand)";
this.collapsed_notice_el.addEventListener("click", this.toggle_fold.bind(this));
this.nav_buttons_container.appendChild(this.collapsed_notice_el);
this.collapse_icon_btn = this.create_icon_button(
"Collapse results",
...
this.collapse_icon_btn.addEventListener("click", this.toggle_fold.bind(this));
this.nav_buttons_container.appendChild(this.collapse_icon_btn);
this.backlink_pane_el = document.createElement("div");
this.backlink_pane_el.classList.add("backlink-pane");
this.backlink_pane_el.style.position = "relative";
this.container_el.appendChild(this.backlink_pane_el);
this.pro_header = document.createElement("div");
this.pro_header.classList.add("tree-item-self", "is-clickable");
this.pro_header.setAttribute("aria-label", "Click to collapse");
this.pro_header.innerHTML = `
...
`;
this.pro_header.addEventListener("click", this.toggle_fold.bind(this));
this.backlink_pane_el.appendChild(this.pro_header);
this.search_result_container = document.createElement("div");
this.search_result_container.classList.add("search-result-container");
this.backlink_pane_el.appendChild(this.search_result_container);
this.connections_pro_wrapper = document.createElement("div");
this.connections_pro_wrapper.classList.add("sc-connections-wrapper");
this.search_result_container.appendChild(this.connections_pro_wrapper);
const cm_container = this.view.dom.querySelector(".cm-contentContainer");
if (cm_container?.parentNode) {
...
localStorage.setItem(local_storage_key, String(this.folded));
if (this.folded) {
this.search_result_container.classList.add("sc-folded");
this.pro_header.classList.add("is-collapsed");
this.collapsed_notice_el.textContent = "Collapsed (click to expand)";
} else {
this.search_result_container.classList.remove("sc-folded");
this.pro_header.classList.remove("is-collapsed");
this.collapsed_notice_el.textContent = "Expanded (click to collapse)";
}
this.render_pro();
}
/**
* Called whenever the editor's state updates.
* We check for set_connections_pro_dom_effect to store the new Pro fragment.
* @param {ViewUpdate} update
*/
update(update) {
for (let tr of update.transactions) {
for (let effect of tr.effects) {
if (effect.is(set_connections_pro_dom_effect)) {
if (effect.value === null) {
this.container_el.style.display = "none";
} else {
this.render_pro(effect.value);
...
/**
* Render the "Smart Connections Pro" area into the wrapper.
*/
render_pro(frag = null) {
if (frag) {
this.connections_pro_frag = frag;
this.connections_pro_wrapper.replaceChildren(this.connections_pro_frag);
}
if (!this.connections_pro_frag) {
this.container_el.style.display = "none";
}
}
/**
* Called when this plugin is destroyed by CodeMirror. Clean up DOM.
*/
destroy() {
this.container_el.remove();
}
/**
* Helper to create an Obsidian-like icon button with the given aria-label and
* inline SVG path code.
* @param {string} label
* @param {string} svg_path
*/
create_icon_button(label, svg_path) {
...
this.opts = opts;
if (!opts.adapter)
throw new Error("HttpRequestAdapter is required");
this.adapter = new opts.adapter(this);
}
/**
...
throw new Error("Obsidian request failed");
...
this._http_adapter = new SmartHttpRequest({ adapter: SmartHttpRequestFetchAdapter });
}
...
if (!entity)
return this.plugin.notices.show("unable_to_init_source", { key: current_file.path });
await entity.import();
...
if (entity.excluded)
return this.plugin.notices.show("item_excluded", { entity_key: entity.key });
...
var SmartNoteInspectModal = class extends import_obsidian9.Modal {
...
var AdvancedConnectionsView = class extends ScConnectionsView {
static get view_type() {
return "smart-advanced-connections-view";
}
static get display_text() {
return "Smart Advanced Connections";
}
static get icon_name() {
return "smart-connections";
}
main_component_key = "advanced_connections";
default_rank_query = "What is the most relevant to the following? Today is {{DATE}}.\n{{CURRENT}}";
get rank_query() {
return typeof this.settings.rank_query === "string" && this.settings.rank_query?.length ? this.settings.rank_query : this.default_rank_query;
}
get should_rank() {
return this.settings.should_rank;
}
get rank_model() {
return this.plugin.rank_model;
}
get rank_model_ready() {
return this.rank_model?.is_loaded && this.rank_model?.adapter?.is_loaded;
}
get settings() {
if (!this.env.settings.connections_pro)
this.env.settings.connections_pro = {};
return this.env.settings.connections_pro;
}
async render_view(entity = null, container = this.container) {
if (container.checkVisibility() === false)
return console.log("View inactive, skipping render nearest");
...
get attribution() {
return super.attribution.replace("Smart Connections", "Smart Connections Pro");
}
get results_container() {
return this.container.querySelector(".sc-list");
}
...
const settings_frag = await this.env.smart_view.render_settings(this.settings_config, { scope: this });
this.settings_container.innerHTML = "";
this.settings_container.appendChild(settings_frag);
const rank_model_frag = await this.env.smart_view.render_settings(this.rank_model.settings_config, { scope: this.rank_model });
this.settings_container.appendChild(rank_model_frag);
return this.settings_container;
}
get settings_config() {
return {
use_rank_model: {
...
type: "toggle",
default: true,
description: "Whether to use the rank model to re-rank the results."
},
rank_query: {
name: "Rank Query",
type: "textarea",
default: this.default_rank_query,
description: "The query to use for ranking. {{DATE}} will be replaced with today's date, and {{CURRENT}} will be replaced with the current context.",
placeholder: this.default_rank_query
}
};
}
};

async function post_process4(view, frag, opts = {}) {
...
* @file connections_pro.css
* @description Styles specific to the Smart Connections Pro layout.
* Fixes the bottom bar so it stays at the bottom of the container.
*/

.sc-connections-view.sc-pro {
display: flex;
...
.sc-connections-view.sc-pro .sc-list {
flex: 1 1 auto;
overflow-y: auto;
padding: 0 8px;
}

.sc-connections-view.sc-pro .sc-bottom-bar {
position: sticky;
bottom: 0;
padding: 6px 8px;
...
`);
var connections_pro_default = css_sheet;

async function build_html2(view, opts = {}) {
...
`).join("");
const html = `<div class="sc-connections-view sc-pro">
...
async function render5(view, opts = {}) {
let html = await build_html2.call(this, view, opts);
const frag = this.create_doc_fragment(html);
this.apply_style_sheet(connections_pro_default);
this.footer_results_container = frag.querySelector(".sc-list");
const header_container = frag.querySelector(".sc-top-bar");
this.sc_footer_top_status_elm = header_container.querySelector(".sc-context");
const footer_container = frag.querySelector(".sc-bottom-bar");
this.sc_footer_bottom_status_elm = footer_container.querySelector(".sc-context");
const entity = await resolve_entity(view);
if (entity === "excluded") {
this.sc_footer_bottom_status_elm.innerText = "File is excluded";
this.sc_footer_top_status_elm.innerText = "File is excluded";
return frag;
}
if (!entity) {
console.log("No entity found, skipping rendering ");
return frag;
}
this.sc_footer_top_status_elm.innerText = `${view.env.smart_sources.keys.length} (${view.env.smart_blocks.keys.length})`;
this.sc_footer_bottom_status_elm.innerText = "Loading...";
const results = await entity.find_connections({
...opts,
exclude_source_connections: entity.env.smart_blocks.settings.embed_blocks
});
const results_frag = await entity.env.render_component("connections_results", results, opts);
this.footer_results_container.innerHTML = "";
Array.from(results_frag.children).forEach((elm) => {
this.footer_results_container.appendChild(elm);
});
this.footer_results_container.dataset.key = entity.key;
const context_name = entity.path.split("/").pop();
this.sc_footer_bottom_status_elm.innerText = context_name;
this.sc_footer_top_status_elm.dataset.key = entity.key;
this.sc_footer_bottom_status_elm.dataset.key = entity.key;
console.log("pre_rank_results", results);
if (view.should_rank && view.rank_model_ready) {
render_ranked_results.call(this, view, entity, results, opts, context_name);
}
return await post_process5.call(this, view, frag, opts);
}
async function render_ranked_results(view, entity, results, opts, context_name) {
this.sc_footer_bottom_status_elm.innerText = "Loading Rank Model...";
await wait_for_model_loaded(view.rank_model);
this.sc_footer_bottom_status_elm.innerText = "Ranking...";
await new Promise((resolve) => setTimeout(resolve, 100));
const query_date = view.rank_query.includes("{{DATE}}") ? (/* @__PURE__ */ new Date()).toLocaleDateString() : null;
const query = view.rank_query.replace("{{CURRENT}}", (await entity.get_embed_input()).slice(0, 500)).replace("{{DATE}}", query_date || "");
const documents = await Promise.all(
results.map(async (r) => {
let doc = "";
if (query_date) {
doc += `modified: ${new Date(r.item.mtime).toLocaleDateString()}
`;
}
doc += await r.item.get_embed_input();
return doc.slice(0, 500);
})
);
if (!documents.length) {
console.log("No documents to rank");
return;
}
console.log("query", query);
console.log("documents", documents);
const ranked_documents = await view.rank_model.rank(query, documents);
console.log("ranked_documents", ranked_documents);
const sorted_results = ranked_documents.map((r) => ({
...results[r.index],
score: r.score
}));
const max_score = Math.max(...sorted_results.map((r) => r.score));
sorted_results.forEach((r) => r.score = r.score / max_score);
const ranked_results_frag = await entity.env.render_component("connections_results", sorted_results, opts);
this.footer_results_container.innerHTML = "";
Array.from(ranked_results_frag.children).forEach((elm) => {
this.footer_results_container.appendChild(elm);
});
console.log("results_container2", this.footer_results_container);
this.sc_footer_bottom_status_elm.innerText = context_name + " (ranked)";
this.footer_results_container.dataset.ranked = "true";
}
async function wait_for_model_loaded(rank_model) {
return new Promise((resolve) => {
const interval = setInterval(() => {
if (rank_model.model_loaded) {
clearInterval(interval);
resolve();
}
}, 100);
...
async function post_process5(view, frag, opts = {}) {
await post_process4.call(this, view, frag, opts);
const advanced_overlay = frag.querySelector(".sc-advanced-overlay");
const advanced_overlay_close = advanced_overlay.querySelector(".sc-advanced-overlay-close");
advanced_overlay_close.addEventListener("click", () => {
advanced_overlay.style.display = "none";
});
return frag;
}
async function resolve_entity(view) {
let entity = null;
const current_file = view.app.workspace.getActiveFile();
if (current_file) {
const key = current_file.path;
const collection = key.includes("#") ? view.env.smart_blocks : view.env.smart_sources;
entity = collection.get(key);
}
if (!entity && current_file) {
console.log("Creating entity for current file", current_file.path);
if (view.env.smart_sources.fs.is_excluded(current_file.path)) {
console.log("File is excluded, skipping");
view.plugin.notices.show("item_excluded", { entity_key: current_file.path });
return "excluded";
}
view.env.smart_sources.fs.include_file(current_file.path);
entity = view.env.smart_sources.init_file_path(current_file.path);
if (!entity)
return view.plugin.notices.show("unable_to_init_source", { key: current_file.path });
await entity.import();
await entity.collection.process_embed_queue();
}
return entity;
...
/**
* @returns {import("obsidian-smart-env").SmartEnv}
*/
get env() {
return this.plugin.env;
}
/**
* Renders the "advanced_connections" component into the footer,
* using this FooterView instance as the render scope.
* If no active editor view is found, the footer remains empty.
*/
async render() {
const view = this.plugin.get_editor_view();
if (!view)
return;
const pro_frag = await this.env.render_component("advanced_connections", this);
view.dispatch({ effects: [set_connections_pro_dom_effect.of(pro_frag)] });
}
/**
* Clears the footer. Removes any existing "advanced_connections" content.
*/
remove() {
const view = this.plugin.get_editor_view();
if (view) {
view.dispatch({ effects: [set_connections_pro_dom_effect.of(null)] });
}
}
/**
* Attempts to refresh the current context and re-render
* similar to sc_connections.obsidian.js.
*/
async refresh() {
const active_file = this.plugin.app.workspace.getActiveFile();
if (active_file) {
const key = active_file.path;
const entity = this.env.smart_sources.get(key);
if (entity) {
await entity.read();
entity.queue_import();
...
var SmartConnectionsEarlyReleasePlugin = class extends import_obsidian10.Plugin {
smart_env_config = {
collections: {},
components: {
advanced_connections: render5
},
modules: {
smart_rank_model: {
class: SmartRankModel,
adapters: {
cohere: SmartRankCohereAdapter
},
http_adapter: new SmartHttpRequest({
adapter: SmartHttpObsidianRequestAdapter,
obsidian_request_url: import_obsidian12.requestUrl
})
}
}
};
/**
* Dynamically load advanced connection views.
...
/**
* Retrieve or create the rank model instance (if enabled).
* @returns {SmartRankModel|null}
*/
get rank_model() {
if (!this.env.settings.connections_pro?.should_rank) {
console.log("rank model disabled");
return null;
}
if (!this._rank_model) {
if (this.env.settings) {
if (!this.env.settings.connections_pro)
this.env.settings_connections_pro = {};
if (!this.env.settings.connections_pro.rank_model)
this.env.settings.connections_pro.rank_model = {};
}
const module_config = this.env.opts.modules.smart_rank_model;
this._rank_model = new module_config.class({
adapters: module_config.adapters,
settings: this.env.settings.connections_pro.rank_model,
use_gpu: false,
reload_model: this.reload_model.bind(this)
});
this._rank_model.adapter.load();
}
return this._rank_model;
}
/**
* Reloads the rank model.
*/
reload_model() {
this._rank_model = null;
this.rank_model?.load();
}
/**
* Simple accessor to plugin notices (if any).
* @returns {any}
*/
get notices() {
return this.env.notices || this.env.smart_connections_plugin?.notices;
}
/**
* @returns {Record<string,any>} short alias to connections_pro settings
*/
get settings() {
if (!this.env.settings.connections_pro)
this.env.settings.connections_pro = {};
...
/**
* Returns whether user has folded the connections (persisted in localStorage).
* @returns {boolean}
*/
isFolded() {
return localStorage.getItem("sc_connections_folded") === "true";
}
onload() {
...
*/
async initialize() {
console.log("Loading Smart Connections Early Release plugin...");
await SmartEnv2.wait_for({ loaded: true });
await SmartEnv2.wait_for({ main: "smart_connections_plugin" });
this.registerEditorExtension(connections_dom_plugin);
this.footer_view = new FooterView(this);
this.registerEvent(
this.app.workspace.on("active-leaf-change", () => {
const view = this.get_editor_view();
if (view && this.settings.footer_connections) {
this.footer_view.render();
}
})
);
this.register_views();
this.addSettingTab(new ScEarlySettingsTab(this.app, this));
if (this.settings.footer_connections) {
const view = this.get_editor_view();
if (view) {
this.footer_view.render();
}
...
*/
onunload() {
console.log("Unloading Smart Connections Early Release plugin...");
this.env?.unload_main?.(this);
if (this.footer_view) {
this.footer_view.remove();
}
}
/**
...
/**
* Dynamically register views for the plugin.
*/
register_views() {
...
/**
* Toggles the "footer_connections" setting. Updates the DOM accordingly.
*/
toggled_footer_connections() {
console.log("toggled_footer_connections");
if (this.settings.footer_connections) {
this.footer_view.render();
} else {
this.footer_view.remove();
...
var ScEarlySettingsTab = class extends import_obsidian13.PluginSettingTab {
...
/**
* @returns {any} reference to the plugin's "smart_view" module if loaded
*/
get smart_view() {
if (!this._smart_view) {
this._smart_view = new this.plugin.env.opts.modules.smart_view.class({
adapter: this.plugin.env.opts.modules.smart_view.adapter
});
}
return this._smart_view;
}
/**
* Renders main settings for the plugin.
* @param {HTMLElement} container
* @param {Object} [opts={}]
* @returns {Promise<HTMLElement>}
*/
...
const plugin_settings_frag = await this.smart_view.render_settings(
{
footer_connections: {
setting: "footer_connections",
name: "Footer Connections",
description: "Show 'Smart Connections Pro' in the footer automatically",
type: "toggle",
callback: "toggled_footer_connections",
default: false
},
should_rank: {
setting: "should_rank",
name: "Should Rank",
description: "Whether to use a rank model to rank the connections",
type: "toggle",
default: false
}
},
...
container.innerHTML = "";
container.appendChild(plugin_settings_frag);
if (this.plugin.rank_model) {
console.log("Rendering rank model settings", this.plugin.rank_model.settings_config);
const rank_model_settings_frag = await this.smart_view.render_settings(
this.plugin.rank_model.settings_config,
{ scope: this.plugin.rank_model }
);
container.appendChild(rank_model_settings_frag);
}
return container;
}
};
